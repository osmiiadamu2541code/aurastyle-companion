...
function ErrorComponent({ error, reset }: { error: Error; reset: () => void }) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        <h1 className="text-xl font-semibold tracking-tight text-foreground">{t("pageLoadFail")}</h1>
        <p className="mt-2 text-sm text-muted-foreground">{t("pageLoadFailBody")}</p>
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Warm daily outfit advice, a smart family wardrobe, and gentle posture and fitness guides.",
      },
      { name: "theme-color", content: "#FFF6EC" },
      { property: "og:title", content: "AuraStyle AI" },
      { property: "og:description", content: "Dressed with love, every single day." },
